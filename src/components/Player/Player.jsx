...
function Player() {
    ...
    return (
        <section className={`w-full flex flex-col items-center md:pb-4 bg-transparent fixed left-0 ${currentTrack ? 'bottom-0' : '-bottom-36'} transition-all duration-500`}>
            <div className='flex w-full md:w-4/6 bg-timberWolf dark:bg-gray-900 rounded-lg px-4 py-3 shadow-content dark:shadow-contentInv flex-wrap'>
                <InputRange NameClass='w-full mb-2 range' ma={duration} val={progress} event={handleTimeChange} />
                <div className='flex w-1/2 md:w-2/6 items-center px-2 py-1 gap-4 flex-grow-1'>
                    ...
                    <div className='overflow-hidden'>
                        <p className='text-lg dark:text-white text-black capitalize overflow-hidden text-ellipsis whitespace-nowrap'>{currentTrack ? currentTrack.name : ''}</p>
                        <Tag nameTag={currentTrack ? currentTrack.tag : ''}>#{currentTrack ? currentTrack.tag : ''}</Tag>
                    </div>
                </div>
                ...
                <div className='hidden md:flex justify-end items-center gap-2 md:w-2/6 flex-grow-1'>
                    ...
                    <InputRange NameClass='w-3/5 range' ma={1} val={mute ? 0 : vol} event={handleVolChange} />
                </div>
            </div>
        </section>
    )
