...
function Modal({ track }) {
    ...
    return (
        <>
            <Boton light={'kepple'} dark={'indigo-800'} textColorLight={'timberWolf'} textColorDark={'indigo-800'} event={handleModal}>
                <FontAwesomeIcon icon={faCircleInfo} />
            </Boton>
            <div className={`absolute w-full flex h-full ${Modal ? 'opacity-100' : 'opacity-0 pointer-events-none '} flex-col px-4 py-2 md:py-3 items-center flex-wrap rounded-lg top-0 left-0 bg-timberWolf dark:bg-gray-900 transition-opacity duration-500`}>
                ...
                <Boton light={'red-500'} dark={'red-500'} textColorLight={'timberWolf'} textColorDark={'white'} event={handleModal}>
                    cerrar
                </Boton>
            </div>
        </>
